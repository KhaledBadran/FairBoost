...
def read_data_baseline(path: Path) -> List[Dict]:
    ...
    for dataset, dataset_results in results["results"].items():
        for preprocessing_method, preprocessing_method_results in dataset_results.items():

            # Baseline experiment without any preprocessing
            if preprocessing_method == 'baseline':
                for classifier, performance_metrics in preprocessing_method_results.items():
                    baseline_results.append(
                        {'experiment': 'baseline', 'bootstrap_type': "No bootstrap", 'dataset': dataset,
                         'preprocessing': ['No Preprocessing'], 'classifier': classifier,
                         'metrics': performance_metrics})

            # Experiments with preprocessing only (e.g., LFR, Reweighing, OptimPrepr)
            elif preprocessing_method != "DisparateImpactRemover":
                for classifier, performance_metrics in preprocessing_method_results[0]["results"].items():
                    baseline_results.append(
                        {'experiment': 'preprocessing', 'bootstrap_type': "No bootstrap", 'dataset': dataset,
                         'preprocessing': [preprocessing_method], 'classifier': classifier,
                         'metrics': performance_metrics})
...
def get_fairboost_run_results(dataset_name: str, raw_run_results: Dict, with_preprocessing: bool) -> List[Dict]:
    ...
    for classifier, performance_metrics in raw_run_results["results"].items():
        run_results.append({'experiment': experiment, 'bootstrap_type': bootstrap_method, 'dataset': dataset_name,
                            'preprocessing': preprocessing_methods, 'classifier': classifier,
                            'metrics': performance_metrics})
...
def read_data_fairboost(path):
    ...
    for dataset, dataset_results in results["results"].items():
        ...
        for run in ensemble_only_results:
            ensemble_run_results = get_fairboost_run_results(dataset_name=dataset, raw_run_results=run,
        ...
        for run in fairboost_results:
            fairboost_run_results = get_fairboost_run_results(dataset_name=dataset, raw_run_results=run,
...
def compute_avg_h_mean(f1_scores: List, normalized_di_scores: List):
    ...
    """
    harmonic_means = list(map(lambda x, y: harmonic_mean([x, y]), f1_scores, normalized_di_scores))
...
def main():
    ...
    df_avg_h_mean = groups['h_mean'].agg([np.mean]).reset_index().sort_values(by=['mean'], ascending=False).rename(columns={"mean": "h_mean"})
    df_avg_f1_score = groups['f1_score'].agg([np.mean]).reset_index().sort_values(by=['mean'], ascending=False).rename(columns={"mean": "f1_score"})
    df_avg_normalized_di = groups['normalized_di'].agg([np.mean]).reset_index().sort_values(by=['mean'], ascending=False).rename(columns={"mean": "normalized_di"})

    new_df = pd.merge(df_avg_h_mean, df_avg_f1_score, how='left', left_on=['experiment','bootstrap_type','dataset', 'preprocessing'], right_on=['experiment','bootstrap_type','dataset', 'preprocessing'])
